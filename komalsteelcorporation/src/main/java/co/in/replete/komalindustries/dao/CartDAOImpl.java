...
public class CartDAOImpl extends BaseDAOImpl implements CartDAO {
	...
	/**
	 * Description : Executes the select query and returns all the cart details
	 * as per the order status code specified
	 * 
	 * @param String
	 *            id of the user
	 * @param String
	 *            status code of the order for which order details are to be
	 *            selected
	...
	public List<CartDetailsTO> selectCartDetaisByOrderStatus(String userMasterId, String orderStatus) {
		List<CartDetailsTO> cartDetailsList = jdbcTemplate.query(
				sqlProperties.getProperty("select.cartdtl.bycartstatuscd"), new String[] { userMasterId, orderStatus },
				new RowMapper<CartDetailsTO>() {
		...
		for (int i = 0; i < cartDetailsList.size(); i++) {
			List<CartItemDetailsListTO> cartItemsList = jdbcTemplate.query(
					sqlProperties.getProperty("select.cartitemdtls.bycartitemdtlsid"),
	...
	public int insertAddress(final ShippingAddressDetail shippingAddressDetail) {
		...
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				PreparedStatement ps = con.prepareStatement(sqlProperties.getProperty("insert.shippingaddress.all"),
	...
	/**
	 * Description : Executes the insert query for cart delivery details and
	 * inserts the cart delivery details
	 * 
	 * @return int Generated Id of the inserted row
	 * @throws {@link
	 *             ServicesException}
	...
	public int insertCartDeliveryDtls(CartDlvryDtl cartDelivryDtls) {
		...
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			@Override
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				PreparedStatement ps = con.prepareStatement(sqlProperties.getProperty("insert.cartdeliverydetails.all"),
						Statement.RETURN_GENERATED_KEYS);

				ps.setInt(1, cartDelivryDtls.getShippingAddressId());
				try {
					ps.setDate(2, (null == cartDelivryDtls.getExpDlvryDt() || cartDelivryDtls.getExpDlvryDt().isEmpty())
							? null : (Date) CommonUtility.getParsedDate(cartDelivryDtls.getExpDlvryDt()));
					ps.setDate(3,
							(null == cartDelivryDtls.getActualDlvryDt() || cartDelivryDtls.getActualDlvryDt().isEmpty())
									? null : (Date) CommonUtility.getParsedDate(cartDelivryDtls.getActualDlvryDt()));
	...
	/**
	 * Description : Executes the insert query for invoice details and inserts
	 * the invoice details
	 * 
	 * @param {@link
	 * 			InvoiceDtlDTO}
	...
	public int insertInvoiceDetails(InvoiceDtl invoiceDetailsParams) {
		...
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				PreparedStatement ps = con.prepareStatement(sqlProperties.getProperty("insert.invoicedetails.all"),
						Statement.RETURN_GENERATED_KEYS);
	...
	/**
	 * Description : Executes the select query for offer details and get details
	 * of the offer associated with the offerDtlsId parameter
	 * 
	 * @param offerDtlsId
	 *            Id of the offer for which details are to be selected
	...
	public OfferDtl selectOfferDetails(String offerDtlsId) {
		return (OfferDtl) jdbcTemplate.query(sqlProperties.getProperty("select.offerdtl.byofferid"),
				new String[] { offerDtlsId }, new BeanPropertyRowMapper<OfferDtl>());
	}

	/**
	 * Description : Executes the insert query for payment details and inserts
	 * the payment details
	 * 
	 * @param {@link
	 * 			PaymentDtlDTO}
	...
	public String insertPaymentDetails(PaymentDtl paymentDtl) {
		...
		jdbcTemplate.update(sqlProperties.getProperty("insert.paymentdtls.all"), uuid, paymentDtl.getTxnRefNo(),
				paymentDtl.getPaymentTs(), paymentDtl.getPaymentModeCd(), paymentDtl.getPaymentStatusCd(),
				paymentDtl.getPaymentGatewayCd(), paymentDtl.getBankRefNum(), paymentDtl.getTxnData(),
				paymentDtl.getPaymentAmount());

		return uuid;
	}

	/**
	 * Description : Executes the insert query for cart details and inserts the
	 * cart details
	 * 
	 * @param {@link
	 * 			CartDtlDTO}
	...
	public int insertCartDetails(CartDtl cartDetailParams) {
		...
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			@Override
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				PreparedStatement ps = con.prepareStatement(sqlProperties.getProperty("insert.cartdetails.all"),
						Statement.RETURN_GENERATED_KEYS);
	...
	/**
	 * Description : Executes the insert query for cart item details and inserts
	 * the cart item details
	 * 
	 * @param {@link
	 * 			List<CartItemDtl>} List of all the cart Item details to be
	 *            inserted
	...
	public int[] insertCartItemDetails(List<CartItemDtl> cartItemDtls) {
		...
		return jdbcTemplate.batchUpdate(sqlProperties.getProperty("insert.cartitemdetails.all"),
	...
	/**
	 * Description : Executes the select query for item details and selects all
	 * the item details as per the filter's provided
	 * 
	 * @param String
	 *            The sql query to be executed
	 * @param String[]
	 *            the array of filter values to be applied to the sql query
	 * @return {@link List<ItemDetailsTO>} List containing the search result
	 *         with details of all the items that matced the search filter
	 * @throws {@link
	 *             ServicesException}
	 * 
	 */
	...
	/**
	 * Description : Returns the list of items that match the given keyword for
	 * category,sub-category and manufacturer
	 * 
	 * @param String
	 *            keyword to search the item details against
	...
	public List<ItemDetailsTO> selectItemsDetails(String keyword) {
		...
		List<ItemDetailsTO> itemsDetails = jdbcTemplate.query(sqlProperties.getProperty("select.itemdetails.bykeyword"),
				new String[] { "%" + keyword + "%", "%" + keyword + "%", "%" + keyword + "%" },
				new RowMapper<ItemDetailsTO>() {
					@Override
					public ItemDetailsTO mapRow(ResultSet rs, int rowNum) throws SQLException {
						ItemDetailsTO itemDetailsTO = new ItemDetailsTO();
						itemDetailsTO.setAvlQty(rs.getBigDecimal("AVL_QTY").toString());
	 ...
	 */
	@Override
	public List<NewCartDetailsTO> selectCartDetaisByPageNum(String trackId, int pageNum, int maxOrders)
			throws DataAccessException {

		int startIdx = (pageNum - 1) * maxOrders;
		return jdbcTemplate.query(sqlProperties.getProperty("select.cartdtls.orderlist"),
				new Object[] { trackId, startIdx, maxOrders }, new RowMapper<NewCartDetailsTO>() {
					@Override
					public NewCartDetailsTO mapRow(ResultSet rs, int rowNum) throws SQLException {
						...
						newCartDetailsTO
								.setLrNoDate((null == rs.getString("LR_NO_DATE")) ? "" : rs.getString("LR_NO_DATE"));
						newCartDetailsTO.setNoOfCartonLoaded((null == rs.getString("NO_OF_CARTON_LOADED")) ? ""
								: rs.getString("NO_OF_CARTON_LOADED"));
	...
	public int selectOrderCount(String trackId) throws DataAccessException {
		return (int) jdbcTemplate.queryForObject(sqlProperties.getProperty("select.cartdtls.ordercount"), Integer.class,
				trackId);
	}
	...
	public ItemsInventoryDtl selectItemInventoryDtl(String itemMasterDtlsId) {
		...
		return jdbcTemplate.queryForObject(sqlProperties.getProperty("select.iteminventorydtls.all"),
				new Object[] { itemMasterDtlsId }, new BeanPropertyRowMapper<>(ItemsInventoryDtl.class));
	}
	...
	public void updateItemInventoryDetails(Float avlQty, Float bookedQty, int itemInventoryDetailsId) {
		...
		jdbcTemplate.update(sqlProperties.getProperty("update.iteminventorydetails"),
				new Object[] { avlQty, bookedQty, itemInventoryDetailsId });
	}

	@Override
	public List<ItemOrderDetailsTO> selectItemDetails(String trackId) throws Exception {
		return jdbcTemplate.query(sqlProperties.getProperty("select.cartitemdtl.itemnamequantity"),
				new Object[] { trackId }, new RowMapper<ItemOrderDetailsTO>() {
					@Override
	...
	public UserDetailsTO selectUserDetails(String trackId) {
		...
		return jdbcTemplate.query(
				"select * from user_dtls ud inner join user_login_dtls uld on ud.TRACK_ID=uld.TRACK_ID and uld.TRACK_ID=?",
				new Object[] { trackId }, new ResultSetExtractor<UserDetailsTO>() {
					@Override
	...
	public List<CartItemDetailsListTO> selectCartItemDtls(int cartDtlsId) {
		...
		return jdbcTemplate.query(sqlProperties.getProperty("select.cartitemdtls.bycartdtlsid"),
				new Object[] { cartDtlsId }, new RowMapper<CartItemDetailsListTO>() {
					@Override
					public CartItemDetailsListTO mapRow(ResultSet rs, int rowNum) throws SQLException {
	...
	public List<WUserOrderInfo> selectUserOrderInfo(String id) {
		return jdbcTemplate.query(sqlProperties.getProperty("select.userorderdetails1.all"), new Object[] { id },
				new RowMapper<WUserOrderInfo>() {
					@Override
	...
	public List<WUserCartDetails> selectUserCartDetails(int invoiceId) {
		return jdbcTemplate.query(sqlProperties.getProperty("select.userinvoice.details"), new Object[] { invoiceId },
				new RowMapper<WUserCartDetails>() {
					@Override
	...
	public List<WOrderDetailsTO> selectOrderDetails() {
		...
		return jdbcTemplate.query(sqlProperties.getProperty("select.orderdetails.all"), new Object[] { wmaxRecords },
				new RowMapper<WOrderDetailsTO>() {
					@Override
	...
	public List<WCartItemTO> selectCartItemDetails(int orderId) {
		return jdbcTemplate.query(sqlProperties.getProperty("select.orderitem.details"), new Object[] { orderId },
				new RowMapper<WCartItemTO>() {
					@Override
	...
	public List<CartItemDtlsTO> selectOrderItemDetailsByCartId(int orderDetailsId) {
		...
		return jdbcTemplate.query(
				"select imd.ITEM_NM,imd.ITEM_MASTER_DTLS_ID,cid.ITEM_QTY,cid.ITEM_PRICE,imd.UOM from item_master_dtls as imd inner join cart_item_dtls as cid on "
	...
	public List<WOrderDetailsTO> searchOrders(String searchBy, String searchDateRange)
		...
		if (searchBy.equals("All")) {
			/*
			 * sql =
			 * "select ud.TRACK_ID,oad.CITY,oad.COUNTRY,cd.CART_DTLS_ID,cdd.EXP_DLVRY_DT,cdd.DLVRY_TYPE,ud.FIRST_NAME,ud.LAST_NAME,cd.CART_NOTES,cd.CREATED_TS,"
			 * +
	...
	public UserDetailsTO selectUserUserDetailsByTrackId(String trackId) {
		...
		List<UserDetailsTO> userDetailsList = jdbcTemplate.query(
				"select * from user_dtls as ud inner join user_login_dtls as uld on ud.TRACK_ID=uld.TRACK_ID and ud.TRACK_ID=?",
	...
	public CartDetailsTO selectOrderDetailsAndDeliveryDetails(int orderId) {
		...
		return jdbcTemplate.queryForObject(sql, new Object[] { orderId },
				new BeanPropertyRowMapper<CartDetailsTO>(CartDetailsTO.class));
	}

	@Override
	public int getCountOfItemsInCart(String orderId) {
		String sql = "SELECT count(*) FROM cart_item_dtls where CART_DTLS_ID=?";
	...
	public List<CartItemDtl> selectCartItemDetailsList(int orderId) {
		String sql = "SELECT * FROM cart_item_dtls where CART_DTLS_ID=?";
		return jdbcTemplate.query(sql, new Object[] { orderId },
				new BeanPropertyRowMapper<CartItemDtl>(CartItemDtl.class));
	}

	@Override
	public void updateCartPrice(int cartDtlsId, float totalChargableAmount2) {

		System.out.println("cartDtlsId = " + cartDtlsId + ", totalChargableAmount2 = " + totalChargableAmount2);
		// sqlProperties.getProperty("update.cartprice")
		String sql = "update cart_dtls set CART_PRICE=? WHERE CART_DTLS_ID=?";
		jdbcTemplate.update(sql, new Object[] { totalChargableAmount2, cartDtlsId });
	}
}
